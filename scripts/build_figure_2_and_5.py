#!/usr/bin/env python3
"""Build release figures for the urban boundary-layer vertical decoupling archive.

Outputs publication-oriented vector and raster bundles for the mechanism diagnostic and the Paris lidar profile-evidence figure.
"""
...
SMII_SRC = ROOT / "data" / "figure_2"
PARIS_SRC = ROOT / "data" / "figure_5"
OUT = ROOT
FIG_DIR = OUT / "figures"
TAB_DIR = OUT / "tables"
REP_DIR = OUT / "reports"
...
def build_fig2() -> tuple[Path, dict[str, object]]:
    required = [
        SMII_SRC / "mechanism_index_hotspot_contrast_v1.csv",
        SMII_SRC / "mechanism_index_upper_state_summary_v1.csv",
        SMII_SRC / "mechanism_index_predictive_benchmark_v1.csv",
    ]
...
def build_fig5() -> tuple[Path, dict[str, object]]:
    required = [
        PARIS_SRC / "paris_lidar_station_metadata.csv",
        PARIS_SRC / "paris_lidar_height_availability_600s.csv",
        PARIS_SRC / "paris_lidar_pair_summary_600s.csv",
        PARIS_SRC / "paris_lidar_event_chain_summary_600s.csv",
        PARIS_SRC / "paris_lidar_event_support_summary_600s.csv",
        PARIS_SRC / "paris_lidar_event_placebo_600s.csv",
    ]
    require_files(required)
